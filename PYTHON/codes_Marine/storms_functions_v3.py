...
def get_filename(filenameFormat,datei):
    ...
    filename = filename.replace('YYYY',datei.strftime('%Y'))
    filename = filename.replace('YY',datei.strftime('%y'))
    filename = filename.replace('MM',datei.strftime('%m'))
    filename = filename.replace('DD',datei.strftime('%d'))
    filename = filename.replace('hh',datei.strftime('%H'))
    filename = filename.replace('mm',datei.strftime('%M'))
    filename = filename.replace('ss',datei.strftime('%S'))
...
def load_swh(path,filename,datei):
    ...
    ds0 = xr.open_dataset(filename_complete)
    
    ds = ds0[['latitude','longitude','time','hs']].sel(time=datei)
...
def distance_vect(lon0,lat0,lons,lats):
    ...
    d[np.abs(c)<1] = EARTH_RADIUS * np.arccos(c[np.abs(c)<1])
...
def detect_storms(field, lon, lat, levels, Npix_min, amp_thresh, d_thresh_min, d_thresh_max,thresh_height_for_scale):
 ...
 #   quick_plot(fieldnan,title='fieldnan')
    ...
    # --- 1. Find all regions with Hs greater than ilev  ---------------
        ...
        # --- 1.0.2 Apply function to select  only one region: origin or duplicate  -----------------------------------------
